# ----- importando csv ------
df_pca_abril <- read.csv('../../app/home/data/treated_data/df_pca_abril.csv')

# ----- Transformando index -----
row.names(df_pca_abril) <- df_pca_abril$Municipio
...
df_pca_abril$CasosNovos_pc <- df_pca_abril$CasosNovos_pc*-1
df_pca_abril$Obitos_pc <- df_pca_abril$Obitos_pc*-1
df_pca_abril$RepasseEstadual_pc <- df_pca_abril$RepasseEstadual_pc*-1

# ----- Matriz de correlacao -----
#gráfico com representação numérica, linha de tendência e distribuições
chart.Correlation(df_pca_abril[6:10], histogram = TRUE, pch = "+")

#salvando matriz
rho_dataframe <- cor(df_pca_abril[6:10]) #lembrar se selecionar apenas as colunas numericas
...
# ----- rodando pca -----
dataframe_std <- df_pca_abril[6:10] %>% 
  scale() %>% 
  data.frame()
...
ggplotly(
  data.frame(afpc_dataframe$rotation) %>%
    mutate(var = names(df_pca_abril[6:10])) %>%
    melt(id.vars = "var") %>%
...
df_pca_abril["Fator1"] <- F1$fator1
df_pca_abril["Fator2"] <- F2$fator2
df_pca_abril["Fator3"] <- F3$fator3
df_pca_abril["Fator4"] <- F4$fator4
df_pca_abril["Fator5"] <- F5$fator5
...
df_pca_abril %>%
  mutate(pontuacao = Fator1 * var_compartilhada[1] +
           ...
           Fator5 * var_compartilhada[5]) -> df_pca_abril

# Visualizando o ranking final
df_pca_abril <- df_pca_abril %>%
  arrange(desc(pontuacao))

# ----- salvando em .csv -----
write.csv(df_pca_abril,'../../app/home/data/treated_data/ranking_abril_21.csv')
