# ----- importando csv ------
df_pca_fevereiro <- read.csv('../../app/home/data/treated_data/df_pca_fevereiro.csv')

# ----- Transformando index -----
row.names(df_pca_fevereiro) <- df_pca_fevereiro$Municipio
...
df_pca_fevereiro$CasosNovos_pc <- df_pca_fevereiro$CasosNovos_pc*-1
df_pca_fevereiro$Obitos_pc <- df_pca_fevereiro$Obitos_pc*-1
df_pca_fevereiro$RepasseEstadual_pc <- df_pca_fevereiro$RepasseEstadual_pc*-1

# ----- Matriz de correlacao -----
#gráfico com representação numérica, linha de tendência e distribuições
chart.Correlation(df_pca_fevereiro[6:10], histogram = TRUE, pch = "+")

#salvando matriz
rho_dataframe <- cor(df_pca_fevereiro[6:10]) #lembrar se selecionar apenas as colunas numericas
...
# ----- rodando pca -----
dataframe_std <- df_pca_fevereiro[6:10] %>% 
  scale() %>% 
  data.frame()
...
ggplotly(
  data.frame(afpc_dataframe$rotation) %>%
    mutate(var = names(df_pca_fevereiro[6:10])) %>%
    melt(id.vars = "var") %>%
...
df_pca_fevereiro["Fator1"] <- F1$fator1
df_pca_fevereiro["Fator2"] <- F2$fator2
df_pca_fevereiro["Fator3"] <- F3$fator3
df_pca_fevereiro["Fator4"] <- F4$fator4
df_pca_fevereiro["Fator5"] <- F5$fator5
...
df_pca_fevereiro %>%
  mutate(pontuacao = Fator1 * var_compartilhada[1] +
           ...
           Fator5 * var_compartilhada[5]) -> df_pca_fevereiro

# Visualizando o ranking final
df_pca_fevereiro <- df_pca_fevereiro %>%
  arrange(desc(pontuacao))

# ----- salvando em .csv -----
write.csv(df_pca_fevereiro,'../../app/home/data/treated_data/ranking_fevereiro_21.csv')
