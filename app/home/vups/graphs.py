...
def kpi_descartados():
    ...
    fig_c2 = go.Figure(
        go.Indicator(
            ...
            delta={"position": "bottom", "reference": resumo.get('n_descartados')},
            domain={"x": [0, 1], "y": [0, 1]},
        )
    )
...
def plot_calendar_heatmap__(cidade="AFONSO CLAUDIO", tipo="NOVOS CASOS", mes_analise=1, ano_analise=2021):
    """
    OBJETIVO:
...
def plot_calendar_heatmap(cidade='AFONSO CLAUDIO', tipo= 'NOVOS CASOS', mes_analise= 1, ano_analise= 2021):
    ...
    if os.path.isfile(os.path.join(const.DATADIR, 'MICRODADOS_tratado.parquet')) == False:
        microdados = vups.datasets.microdados()
        vups_utils.tratando_microdados(microdados)
    df_calendar = pd.read_parquet(os.path.join(const.DATADIR, 'MICRODADOS_tratado.parquet'))
...
def plot_tributos_ipca___(cidade="AFONSO CLAUDIO"):
